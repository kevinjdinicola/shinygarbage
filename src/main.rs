...
fn update(_app: &App, model: &mut Model, _update: Update) {
    ...
    model.time_delta = model.cur_frame_time
        .duration_since(model.prev_frame_time).unwrap().as_millis();
    ...
    let (x1, y1, x2, y2) = (model.prev_mouse.x, model.prev_mouse.y, model.cur_mouse.x, model.cur_mouse.y);
    ...
    let did_bonk_new_rect = model.focused_rect.0 != cur_bonked_rect.0 || model.focused_rect.1 != cur_bonked_rect.1;
...
fn update_rects(app: &App, model: &mut Model) {
    ...
    let (rect_width, rect_height) = (win_width / RECT_COUNT_X as f32, win_height / RECT_COUNT_Y as f32);
